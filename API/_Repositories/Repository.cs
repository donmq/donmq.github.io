...
namespace API._Repositories
{
    public class Repository<T> : IRepository<T> where T : class
    {
        ...
        public async Task<T> FirstOrDefaultAsyncAsNoTracking(Expression<Func<T, bool>> predicate, params Expression<Func<T, object>>[] includeProperties)
        {
             return await QueryableEntity(includeProperties).AsNoTracking().FirstOrDefaultAsync(predicate);
        }
    }
}