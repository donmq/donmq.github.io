...
namespace eTierV2_API.Helpers.Utilities
{
        ...
        public static ExcelResult CheckExcel(IFormFile file, string subPath)
        {
            ...
            foreach (Worksheet ws in designer.Workbook.Worksheets)
            {
                ...
                if (ws.Cells.MaxDataColumn < wsTemp.Cells.MaxDataColumn)
                    return new ExcelResult(false, $"Not enough column for data import sheet {ws.Name}");
                if (ws.Cells.MaxDataColumn > wsTemp.Cells.MaxDataColumn)
                    return new ExcelResult(false, $"Higher column quantity than required sheet {ws.Name}");
                if (ws.Cells.MaxDataRow <= wsTemp.Cells.MaxDataRow)
                    return new ExcelResult(false, $"No data in excel file sheet {ws.Name}");
                ...
                for (int r = 0; r < rangeTemp.RowCount; r++)
                {
                    for (int c = 0; c < rangeTemp.ColumnCount; c++)
                    {
                        ...
                        if (val != valTmp)
                            return new ExcelResult(false, $"Header in cell {CellsHelper.CellIndexToName(r, c)} : {val}\nMust be : {valTmp} sheet {ws.Name}");
                    }
                }
            }
            return new ExcelResult(true, new Workbook(stream), temp);
        }
    ...
    public class ExcelResult
    {
        ...
        public Workbook wb { set; get; }
        public Workbook wbTemp { set; get; }
        ...
        public ExcelResult(bool isSuccess, Workbook workbook, Workbook workbookTemp)
        {
            IsSuccess = isSuccess;
            wb = workbook;
            wbTemp = workbookTemp;
    ...
    public class Cell
    {
        ...
        public Cell(string location, string value, Style style)
        {
            Location = location;
            Value = value;
            Style = style;
        ...
        public Cell(int row, int column, string value, Style style)
        {
            Location = CellsHelper.CellIndexToName(row, column);
            Value = value;
            Style = style;
